...
\section{Boolean valued semantics for infinitary logics [CORSO]}
\label{sec:org0528a65}
\maketitle
...
\subsection{Lecture 1 -  \textit{<2025-11-17 Lun>}}
\label{sec:orga21c3b1}
...
\subsubsection{Tarski semantics for \(\L_{\kappa,\lambda}\)}
\label{sec:org74968f4}
...
\end{esempio}
\subsection{Lecture 2 - \textit{<2025-11-19 Mer>}}
\label{sec:org7d2671a}
...
\end{equation*}
which is the following, from the rules in in Section 3.2
\begin{prooftree}
...
\subsubsection{Failure of interpolation for Tarski semantics on \(\L_{\infty,\omega}\) and \(\L_{\infty,\infty}\).}
...
\end{equation*}
\end{proof}
\subsubsection{Boolean valid semantics}
\label{sec:org8b8b72b}
...
\end{enumerate}
\end{definizione}
\subsection{Lecture 3 - \textit{<2025-11-24 Lun>}}
\label{sec:org8969ac6}

Last lecture:
\begin{enumerate}
\item Proof that interpolation fail for \(\L_{\infty,\omega}\).
\item Failure of completeness for Tarski semantics with regard to the Proof System in .
...
\end{equation*}
while no \(\M\vDash T\).
\subsubsection{Boolean valued semantics}
\label{sec:org10307cc}
...
If \(B=2 = \set{0,1}\), then the \(B\)-valued models are exactly the Tarski models. The rules are given by the equality axioms in Section 3.2 in .
...
\end{quote}
\end{definizione}
\subsection{Lecture 4 - \textit{<2025-11-26 Mer>}}
\label{sec:org0d92801}
...
\begin{oss}
For any info about boolean algebras, see , Chapters 3 and 4.
\end{oss}
...
proves \(1.\Leftrightarrow 2.\) for \(\L_{\infty,\infty}\).
...
\subsection{Lecture 5 - \textit{<2025-12-01 Lun>}}
\label{sec:org91d82e1}
