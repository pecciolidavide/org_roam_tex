...
\section{Unione disgiunta}
\label{sec:org8949b14}
\subsection{Unione disgiunta di insiemi}
...
\subsection{Unione disgiunta di spazi topologici}
\label{sec:orgd554458}
\begin{definizione}
Sia \(\{(X_i, \uptau_i)\}_{i \in I}\) una famiglia indicizzata di \href{20250103145124-topologia.org}{spazi topologici}.
L'\textbf{\textbf{unione disgiunta}} (o somma topologica) è l'insieme \(X\) definito come l'\hyperref[sec:org8949b14]{unione disgiunta insiemistica}:
\begin{equation*}
X = \coprod_{i \in I} X_i := \bigcup_{i \in I} (X_i \times \{i\})
\end{equation*}
dotato della \textbf{\textbf{topologia dell'unione disgiunta}} \(\uptau\).
Un sottoinsieme \(U \subseteq X\) è definito aperto in \(X\) (cioè \(U \in \uptau\)) se e solo se la sua intersezione con ogni spazio componente è aperta nella topologia originale di quello spazio. Formalmente:
\begin{equation*}
U \in \uptau \iff \forall i \in I, \quad U \cap X_i \in \uptau_i
\end{equation*}
...
\begin{prop}
Sia \(X = \coprod_{i \in I} X_i\) l'unione disgiunta della famiglia di spazi topologici \(\{(X_i, \uptau_i)\}_{i \in I}\) e siano \(\iota_j: X_j \to X\) le iniezioni canoniche per ogni \(j \in I\).
