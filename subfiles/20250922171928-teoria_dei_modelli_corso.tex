...
\begin{thm}
(\(T\) stabile).
Ogni \(q(x) \in S(\acl^{\eq} A)\)\footnote{\(q(x)\) è un tipo completo a parametri in \(S(\acl^{\eq} A)\).\label{org94f5c3d}} è stazionario su \(A\).
...
\uline{Esercizio}:
(\(T\) stabile).
Ogni \(q(x) \in S(M)\)\textsuperscript{\ref{org94f5c3d}} è stazionario su \(M\).
