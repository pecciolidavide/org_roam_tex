...
\date{\today}
\title{Geometria Superiore [CORSO]}
\begin{document}

\section{Prima Parte}
\label{sec:orgc29f2d5}
...
\item Coomologia di De Rham di \(\R\) (Esempio~5.1.3 di \autocite{abateGeometriaDifferenziale2011}) e \(\R^n\) (Corollario~5.3.6 di \autocite{abateGeometriaDifferenziale2011}, \emph{non dimostrato}).
\item Diffeomorfismo induce \(k\)-esimi gruppi di omologia isomorfi.
\item Pullback e proprietà (Definizione~4.1.4 e Proposizione~4.1.5 di \autocite{abateGeometriaDifferenziale2011}, \emph{non dimostrata}).
\item Esempio~5.1.19 di \autocite{abateGeometriaDifferenziale2011}.
\item Definizione~5.1.6 di \autocite{abateGeometriaDifferenziale2011} e paragrafo precedente.
\item Definizione~5.1.20 e Osservazione~5.1.21 di \autocite{abateGeometriaDifferenziale2011}.
...
\item Teorema~5.1.23 di \autocite{abateGeometriaDifferenziale2011}.
\item Sezione~5.2 di \autocite{abateGeometriaDifferenziale2011}:
Teorema~5.2.1, Definizione~5.2.2, Osservazione~5.2.3.
\item Sezione~26.2 di \autocite{tuIntroductionManifolds2011a} (Coomologia di \(\mathds{S}\)).
\item Problema~26.2 (pagina 295) di \autocite{tuIntroductionManifolds2011a}.
\item Esempio~5.3.7 di \autocite{abateGeometriaDifferenziale2011}, (vedi anche Teorema~5.3.5 di \autocite{abateGeometriaDifferenziale2011}, \emph{non dimostrato}).
\item Definizione~4.2.6, Osservazione~4.2.9 e Proposizione~4.2.10 (\emph{non dimostrata}), Esempio~4.2.13 di \autocite{abateGeometriaDifferenziale2011}.
\item Definizione~4.3.5 di \autocite{abateGeometriaDifferenziale2011} (Integrazione di forme differenziali a supporto compatto).
\item Ruolo della forma volume nel calcolo di \(H^{n}_{\text{dR}}(\mathds{S}^{n})\) tramite il Teorema di Stokes (Teorema~4.5.12 e Corollario~4.5.15(1) di \autocite{abateGeometriaDifferenziale2011}).
\item \(\int_{\mathds{S}^{n}} : \operatorname{H}^{n}(\mathds{S}^{n})\to \R\) è isomorfsimo: ultime 7 righe di pagina 259 di \autocite{abateGeometriaDifferenziale2011}.
\item Teorema di invarianza omotopica per la coomologia di de Rham: Corollario~5.4.7 di \autocite{abateGeometriaDifferenziale2011} (\emph{non dimostrato}).
\item Definizione di spazi omotopicamente equivalenti (Definizioni~5.4.1 e 5.4.4 di \autocite{abateGeometriaDifferenziale2011}).
\item Calcolo della coomologia di \(\mathds{P}^{n}(\R)\): Esempio~5.3.8 di \autocite{abateGeometriaDifferenziale2011}.
\item Calcolo della coomologia del Toro: Sezione~28.1 di \autocite{tuIntroductionManifolds2011a}.
\item Esempio 4.2.14 di \autocite{abateGeometriaDifferenziale2011}.
\item Ultime 3 righe di pagina 259 di \autocite{abateGeometriaDifferenziale2011}:
\begin{quote}
...
\item Formula di Kunnet: Teorema~5.7.5 di \autocite{abateGeometriaDifferenziale2011} (\emph{non dimostrato}).
\begin{itemize}
...
\end{itemize}
\item Definizione~5.8.1 di \autocite{abateGeometriaDifferenziale2011}
\item Teorema \emph{non dimostrato}: ogni varietà differenziabile ammette un ricoprimento aciclico. (Teorema 5.8.8 di \autocite{abateGeometriaDifferenziale2011}).
\item Proposizione~5.8.3 di \autocite{abateGeometriaDifferenziale2011}.
\item Definizione~5.5.1, Osservazione~5.5.2 e Esempio~5.5.3 di \autocite{abateGeometriaDifferenziale2011}.
...
\begin{equation*}
  ...
  \end{cases}
\end{equation*}
\item Corollario~5.5.12 di \autocite{abateGeometriaDifferenziale2011} (\emph{non dimostrato}).
\item Osservazione~5.5.13 di \autocite{abateGeometriaDifferenziale2011}.
...
\item Lemma~5.1.22 di \autocite{abateGeometriaDifferenziale2011}.
\item Definizione~5.5.7, ``successione di Mayer-Vietoris a supporto compatto'' e Teorema~5.5.8 (\emph{dimostrata solo suriettività di \(s\)}) di \autocite{abateGeometriaDifferenziale2011}.
\item \emph{Proposizione}: se \(M,N\) sono diffeomorfe, allora hanno coomologia a supporto compatto isomorfe (se \(F\) è diffeomorfismo allora \(F\) è propria: vedi Osservazione~5.5.6 di \autocite{abateGeometriaDifferenziale2011}).
\item Corollario~5.6.7 e Osservazione~5.6.8 di \autocite{abateGeometriaDifferenziale2011}.
\item Dualità di Poincaré (da \autocite{abateGeometriaDifferenziale2011}):
\begin{itemize}
...
\end{itemize}
\item Corollario~5.6.9 di \autocite{abateGeometriaDifferenziale2011}.
...
\item Corollario~4.5.15(ii) di \autocite{abateGeometriaDifferenziale2011}.
\item Calcolo delle superfici compatte orientabili \(S_{g}\): \autocite{galluzziCoomologiaDiRham2025} (vedi anche la Sezione~28.2 di \autocite{tuIntroductionManifolds2011a}).
\item Esercizio~5.27 e Esercizio~5.28 di \autocite{abateGeometriaDifferenziale2011} (risolti in classe, vedi \autocite{galluzziGradoDiMappa2025}).
\end{itemize}
...
\label{sec:org6908f2e}

\subsection{Lezione 1}
\label{sec:org878eab9}
...
\end{equation*}
(vedi \hyperref[sec:orgb010d20]{Funzioni Cinfinito tra varietà differenziabili})
\end{definizione}
...
\begin{itemize}
\item una \hyperref[sec:orgb010d20]{funzione \(C^{\infty}\)}
...
Inoltre \(x^{i}:U\to \R: p\mapsto x^{i}_{p}\) sono \hyperref[sec:orgb010d20]{funzioni \(C^{\infty}(U)\)}.
\end{esempio}
\section{Forma differenziale in un punto}
\label{sec:org85d06f7}
\lipsum[1]
\section{Forma differenziale}
\label{sec:org7afd152}
\lipsum[2]

Anche \(\Omega^{\bullet}(M)\) graduato.
\section{Prodotto wedge tra forme differenziali}
\label{sec:org5da3989}
\lipsum[3]
...
\label{sec:org93adfa7}
\def\d{\operatorname{d}}

Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile}, e si indichi con \(\Omega^{k}(M)\) l'insieme delle \hyperref[sec:org7afd152]{\(k\)-forme differenziali} su \(M\).

\begin{definizione}
Il \uline{differenziale} è una funzione
\(\dif :\Omega^{k}(M) \to \Omega^{k+1}(M)\) definita come segue:
\begin{itemize}
\item se \(k = 0\), allora \(\Omega^{0}(M) = C^{\infty}(M)\)\footnote{Questo è l'\hyperref[sec:orgc7903cf]{anello delle funzioni \(C^{\infty}\) da una varietà ai reali}.} e quindi, localmente,
\begin{equation*}
...
\end{itemize}
questa definizione è valida per tutti questi esempi. Si noti che \textbf{non è valida} per le \hyperref[sec:org87af76a]{Algebre Graduate}.
\end{oss}
\begingroup
\footnotesize
\section{Gruppo di Coomologia di De Rham}
\label{sec:orga6f5267}
Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile} di dimensione \(n\), e sia \(\Omega^{k}(M)\) lo \href{20241205142027-spazio_vettoriale.org}{spazio vettoriale} delle \hyperref[sec:org7afd152]{\(k\)-forme}. Si indichi con
\begin{equation*}
\d^{k} : \Omega^{k}(M) \to \Omega^{k+1}(M)
\end{equation*}
il \hyperref[sec:org93adfa7]{differenziale di forme}.

\begin{oss}
Si ha che \(\operatorname{rng} \d^{k-1} \subseteq \ker \d^{k}\)\footnote{Vedi ``\href{20251121143525-kernel_di_una_funzione_tra_spazi_vettoriali.org}{Kernel di una funzione tra spazi vettoriali}'' e ``\href{20250202173528-dominio_range_e_campo_di_una_classe_relazione.org}{Range di una funzione}''}, per le \hyperref[sec:org7044530]{proprietà del differenziale di forme}.
\end{oss}
\begin{definizione}
...
\end{equation*}
e questo è uno \uline{\hyperref[sec:org141d91e]{spazio vettoriale graduato}}.
\end{definizione}
\begin{oss}
Si ha che \(\H^{k}(M) = 0\) sse tutte le \(k\)-forme \hyperref[sec:orgc77f16f]{chiuse} sono \hyperref[sec:org45e681c]{esatte}.
\end{oss}
\begin{prop}
...
\label{sec:orgc77f16f}
\def\d{\operatorname{d}}
\def\H{\operatorname{H}_{\text{dR}}}

Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile}, e sia \(\Omega^{k}(M)\) lo \href{20241205142027-spazio_vettoriale.org}{spazio vettoriale} delle \hyperref[sec:org7afd152]{\(k\)-forme}. Si indichi con \(\d\) il \hyperref[sec:org93adfa7]{differenziale di forme}.

\begin{definizione}
Una forma \(\omega \in \Omega^{k}(M)\) è \uline{chiusa} se \(\dif \omega = 0\)
\end{definizione}
\section{Forma differenziale esatta}
\label{sec:org45e681c}
\def\d{\operatorname{d}}
\def\H{\operatorname{H}_{\text{dR}}}
...
\begin{definizione}
Un \textbf{campo vettoriale} su \(U\) è una \hyperref[sec:orgb010d20]{funzione \(C^{\infty}\)}(infatti \href{20250113152503-aperti_di_una_varieta_differenziabile_sono_varieta_differenziabili.org}{Aperti di una varietà differenziabile sono varietà differenziabili})
\begin{align*}
...
\label{sec:org5b09938}

\def\d{\operatorname{d}}
\def\H{\operatorname{H}_{\text{dR}}}

\(\R\) è una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile} di dimensione \(1\). Pertanto esistono \hyperref[sec:org7afd152]{\(k\)-forme} solo per \(k=0,1\). Pertanto dobbiamo calcolare solo la \hyperref[sec:orga6f5267]{Coomologia di De Rham}:
\begin{equation*}
\H^{0}(\R),\qquad \H^{1}(\R)
\end{equation*}
...
\end{equation*}
Sia quindi \(f \in \Omega^{0}(\R) = C^{\infty}(\R)\)\footnote{Questo è l'\hyperref[sec:orgc7903cf]{anello delle funzioni \(C^{\infty}\)}.} tale che \(\dif f = 0\), \(\dif f = 0\) sse \(f' = 0\) sse \(f\) è costante (poiché il dominio è connesso).
...
\label{sec:org274bcf6}
\def\d{\operatorname{d}}
\def\H{\operatorname{H}_{\text{dR}}}

\begin{thm}
(Lemma di Poincaré)
Sia \(\H^{k}(\R^{n})\) la \hyperref[sec:orga6f5267]{coomologia di De Rham} di \(\R^{n}\)\footnote{Ovviamente \(\R^{n}\) ha una struttura di \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile}.}, per \(n>1\). Allora
\begin{equation*}
\H^{k}(\R^{n}) = %
\begin{cases}
\R & k=0\\
...
\label{sec:orgfd4a0e2}
Siano \(M,N\) due \href{20250113115909-struttura_differenziabile.org}{varietà differenziabili}, \(F:M\to N\) una \hyperref[sec:orgb010d20]{funzione \(C^{\infty}\)}, e \(\omega \in \Omega^{n}(N)\) una \hyperref[sec:org7afd152]{\(n\)-forma} su \(N\).
...
\label{sec:org204f778}
\begin{prop}
Siano \(M,N\) due \href{20250113115909-struttura_differenziabile.org}{varietà differenziabili}, \(F:M\to N\) una \hyperref[sec:orgb010d20]{funzione \(C^{\infty}\)}.
\begin{enumerate}
\item \(F^{*}:\Omega^{n}(N) \to \Omega^{n}({M})\)\footnote{\(\Omega^{n}(N)\) lo spazio delle \hyperref[sec:org7afd152]{Forme differenziali} su \(N\).} è una \href{20250114101949-funzione_lineare.org}{funzione lineare} per ogni \(n\ge 0\), e induce per linearità una funzione
\begin{equation*}
 F^{*}:\Omega^{\bullet}(N) \to \Omega^{\bullet}(M).
\end{equation*}
\item Per ogni \(\omega, \psi \in \Omega^{\bullet}(N)\):\footnote{\(\wedge\) è il \hyperref[sec:org5da3989]{Prodotto wedge tra forme differenziali}}
\begin{equation*}
...
\end{equation*}
dove \(\operatorname{d}\) è il \hyperref[sec:org93adfa7]{differenziale di forme}.
\item Se \(P\) è una varietà differenziabile, \(G: N \to P\) è una funzione \(C^{\infty}\), allora
\begin{equation*}
 (G\circ F)^{*} = F^{*}\circ G^{*}.
\end{equation*}

\item Se \(F\) è un \hyperref[sec:org0bd9510]{diffeomorfismo} allora \(F^{*}: \Omega^{\bullet}(N) \to \Omega^{\bullet}(M)\) è un \hyperref[sec:orgd97769d]{isomorfismo di algebre graduate}, e \((F^{*})^{-1} = (F^{-1})^{*}\)
\end{enumerate}
\end{prop}
\section{0-gruppo di coomologia di de Rham di una varietà connessa}
\label{sec:org0c3a6a2}
Sia \(M\) una varietà differenziabile, e sia \(\operatorname{H}^{0}(M)\) lo \(0\)-\hyperref[sec:orga6f5267]{gruppo di coomologia di De Rham} di \(M\).
\begin{itemize}
...
\label{sec:orgd97769d}
\begin{definizione}
Siano \(V=\bigoplus_{i \in \N} V_{i}\), \(W = \bigoplus_{j \in \N} W_{j}\) due \href{20250110175552-algebra_su_un_campo.org}{algebre} \hyperref[sec:org87af76a]{graduate}.
\begin{itemize}
...
\label{sec:orgc9c22e8}
Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile} e sia \(\HdRham^{\bullet}(M)\) la \href{20241213095808-somma_diretta.org}{somma diretta} dei \hyperref[sec:orga6f5267]{gruppi di coomologia di De Rham}.
\begin{definizione}
...
\end{align*}
dove \(\omega\wedge\eta\) è il \hyperref[sec:org5da3989]{prodotto wedge tra forme differenziali}.
\end{definizione}

\begin{oss}
Questo prodotto rende \(\HdRham^{\bullet}(M)\) un'\href{20250110175552-algebra_su_un_campo.org}{algebra (reale)} \hyperref[sec:org87af76a]{graduata} \href{20250110175552-algebra_su_un_campo.org}{associativa} e \href{20250110175552-algebra_su_un_campo.org}{anticommutativa}.
\end{oss}
\section{Pullback di una funzione tra varietà differenziabili in coomologia}
\label{sec:orgb35e1d7}
\def\H{H_{\text{dR}}}

Siano \(M,N\) due \href{20250113115909-struttura_differenziabile.org}{varietà differenziabili}, \(F:M\to N\) una \hyperref[sec:orgb010d20]{funzione \(C^{\infty}\)}.

\begin{definizione}
Poiché il \hyperref[sec:orgfd4a0e2]{pullback} commuta con il \hyperref[sec:org93adfa7]{differenziale}, \(F\) induce un \hyperref[sec:orgd97769d]{morfismo} tra \hyperref[sec:org87af76a]{algebre graduate}:
\begin{align*}
...
\label{sec:orgaa5418d}
Siano \(M, N\) due \href{20250113115909-struttura_differenziabile.org}{varietà differenziabili} di dimensione, rispettivamente, \(m\) ed \(n\). Si indichi con \(\HdRham\) la \hyperref[sec:orga6f5267]{coomologia di De Rham}.
\begin{thm}
Se \(F:M\to N\) è un \hyperref[sec:org0bd9510]{diffeomorfismo} tra \href{20250113115909-struttura_differenziabile.org}{varietà differenziabili}, allora per ogni \(k \in \N\)  si ha l'\href{20250113125833-isomorfismo_tra_spazi_vettoriali.org}{isomorfismo tra spazi vettoriali}:
\begin{equation*}
...
\begin{proof}
Fissato \(k \in \N\), si consideri il \hyperref[sec:orgfd4a0e2]{pullback} sulle \hyperref[sec:org7afd152]{forme}:
\begin{equation*}
F^{*}:\Omega^{k}(N) \to \Omega^{k}(M)
\end{equation*}
\hyperref[sec:org204f778]{che soddisfa} \(\dif F^{*}\omega = F^{*} \dif \omega\) (dove \(\dif\) è la \hyperref[sec:org93adfa7]{derivata esterna}).
\begin{itemize}
\item Se \(\omega \in \Omega^{k}(N)\) è \hyperref[sec:orgc77f16f]{chiusa}, allora \(\dif \omega = 0\)
\begin{equation*}
  \dif F^{*}\omega = F^{*}\dif \omega = 0
\end{equation*}
e pertanto \(F^{*}\omega\) è chiusa.
\item Se \(\omega \in \Omega^{k}(N)\) è \hyperref[sec:org45e681c]{esatta}, allora esiste \(\eta\) tale che \(\omega = \dif\eta\):
\begin{equation*}
...
\end{proof}
\begin{thm}
Se \(F:M\to N\) è un \hyperref[sec:org0bd9510]{diffeomorfismo} tra \href{20250113115909-struttura_differenziabile.org}{varietà differenziabili}, allora si ha l'\hyperref[sec:orgd97769d]{isomorfismo tra algebre graduate}:
\begin{equation*}
...
\label{sec:org51522b3}
\begin{definizione}
Sia \(A^{\bullet} \coloneqq \left\langle (A^{k}, \operatorname{d}^{k} )\right\rangle_{k \in \Z}\) un \hyperref[sec:orgf743f72]{complesso di cocatene}.
...
\label{sec:org39cbe10}
\begin{definizione}
Siano \(C^{\bullet} \coloneqq \left\langle (C^{k}, \mathrm{d}_{C}^{k})\right\rangle_{k \in \Z}\) e \(D^{\bullet} \coloneqq \left\langle (D^{k}, \mathrm{d}_{D}^{k})\right\rangle_{k \in \Z}\) due \hyperref[sec:orgf743f72]{complessi di cocatene}. Un \uline{morfismo tra complessi di cocatene} \(F^{\bullet} : C^{\bullet} \to D^{\bullet}\) è una \href{20250115100904-successione.org}{successione} \(F^{\bullet} \coloneqq \langle F^{k} \rangle_{k \in \Z}\) tale che per ogni \(k \in \Z\):
\begin{itemize}
...
\end{equation*}
due \hyperref[sec:orgf743f72]{complessi di cocatene} e sia \(F^{\bullet} : C^{\bullet} \to D^{\bullet}\) un \hyperref[sec:org39cbe10]{morfismo}, \(F^{\bullet} = \langle F^{k}\rangle_{k \in \Z}\).

Allora, per ogni \(k \in \Z\), \(F^{\bullet}\) induce un morfismo tra i \hyperref[sec:org51522b3]{gruppi di coomologia}:
\begin{align*}
...
\item tutte le righe sono \hyperref[sec:org233b1bc]{successioni esatte corte di spazi vettoriali}.
\end{itemize}
\begingroup
\scriptsize
\section{Zig-Zag Lemma (per complessi di cocatene)}
\label{sec:orgdb6d57a}
\begin{prop}
Sia
\begin{equation*}
\begin{tikzcd}
...
\end{tikzcd}
\end{equation*}
una \hyperref[sec:orgf637da5]{SEC} di \hyperref[sec:orgf743f72]{complessi di cocatene}. Allora esiste \(\delta\), detto ``morfismo di connessione'', tale che\footnote{Nota:
\begin{equation*}
H^k(C^\bullet) = \frac{\ker d_C^k}{\operatorname{Im} d_C^{k-1}}
\end{equation*}
Questa è la \hyperref[sec:org51522b3]{Coomologia di un complesso di cocatene}.}
\begin{equation*}
...
\end{tikzcd}
\end{equation*}
è una \hyperref[sec:orgf637da5]{successione esatta lunga}.
\end{prop}
...
\end{tikzcd}
\end{equation*}
È possibile farne il \hyperref[sec:orgfd4a0e2]{pullback} alle \hyperref[sec:org7afd152]{forme differenziali} per ogni \(k \in \N\), ottenendo:
\begin{equation*}
\begin{tikzcd}
	& {A^k(U_0)} \\
...
\begin{thm}
Questa \hyperref[sec:org233b1bc]{successione è esatta}.
\end{thm}
\begin{proof}
...
Si utilizza una partizione dell'unità subordinata al ricoprimento \(\mathcal{U} = \set{U_{0}, U_{1}}\): esistono \hyperref[sec:orgc7903cf]{due funzioni} \(\rho_{0},\rho_{1} \in C^{\infty}(M) = A^{0}(M)\)  tali che i loro \href{20250701115005-supporto_di_una_funzione.org}{supporti}
\begin{align*}
...
\begin{oss}
È possibile vedere questa come una \hyperref[sec:orgf637da5]{successione} di \hyperref[sec:orgf743f72]{complessi di cocatene}
\end{oss}

\begin{oss}
Tramite lo \hyperref[sec:orgdb6d57a]{Zig-Zag Lemma}, otteniamo una \hyperref[sec:org233b1bc]{successione esatta} in \hyperref[sec:org51522b3]{coomologia} (\href{20250122122650-quoziente_di_somma_diretta_di_moduli.org}{somma diretta commuta con quoziente}):
\begin{equation*}
\begin{tikzcd}
	\dots \\
...
\end{tikzcd}
\end{equation*}
ed è possibile calcolare esplicitamente il morfismo di connessione \(\partial^{*}\) (\hyperref[sec:org69bfd83]{oltre che gli altri morfismi}), seguendo la costruizione: dato \([\eta] \in H^{k}(U_{0}\cap U_{1})\),
\begin{equation*}
\eta = (\iota_{1}^{*}-\iota_{0}^{*})(-\rho_{1} \eta, \rho_{0} \eta)
\end{equation*}
Facendo il differenziale di \((-\rho_{1} \eta, \rho_{0} \eta)\) su ambo le componenti, si \hyperref[sec:org7044530]{ottiene}
\begin{equation*}
(- \dif \rho_{1} \wedge \eta, \dif \rho_{0} \wedge \eta)
\end{equation*}
in quanto \(\dif\eta = 0\)\footnote{Vedi forme \hyperref[sec:orgc77f16f]{chiuse} ed \hyperref[sec:org45e681c]{esatte}.}. Ponendo ora
\begin{equation*}
\omega = %
\begin{cases}
...
\end{tikzpicture}
\end{equation*}
È una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile} di dimensione 1, quindi si vogliono calcolare la \hyperref[sec:orga6f5267]{coomologia} \(H^{0}(\mathds{S}^{1})\) e \(H^{1}(\mathds{S}^{1})\).
...
\end{align*}
dove gli ultimi sono \hyperref[sec:org0bd9510]{diffeomorfismi}. Utilizzando \hyperref[sec:org87251ff]{Mayer-Vietoris} si ottiene la seguente successione esatta:
\begin{equation*}
...
\end{equation*}
\begin{itemize}
\item Siccome la \hyperref[sec:orgaa5418d]{coomologia è invariante per diffeomorfismi}, si ottiene
\begin{equation*}
\begin{tikzcd}[column sep=scriptsize]
...
\end{tikzcd}
\end{equation*}
\item Ricordando \hyperref[sec:org0c3a6a2]{come funziona lo \(0\) gruppo di coomologia}:
\begin{equation*}
\begin{tikzcd}[column sep=scriptsize]
...
\label{sec:org2883e56}
\begin{thm}
Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile}, e sia
\begin{equation*}
\pi: M\times \R \to M
\end{equation*}
la proiezione. Allora \(\pi\) induce un \hyperref[sec:orgb35e1d7]{pullback}:
\begin{equation*}
\pi^{*}: H^{\bullet}(M\times \R) \xrightarrow{\cong} H^{\bullet}(M)
\end{equation*}
che è un \hyperref[sec:orgd97769d]{isomorfismo} tra le \hyperref[sec:orgc9c22e8]{coomologie}.
\end{thm}
\section{Coomologia delle sfere}
\label{sec:orge78e440}
\begin{thm}
Sia \(\mathds{S}^{n}\) la \href{20250115150754-sfera_n_dimensionale.org}{sfera \(n\)-dimensionale}. Allora la sua \hyperref[sec:orga6f5267]{coomologia di De Rham} è:
\begin{equation*}
H^{k}(\mathds{S}^{n}) = %
\begin{cases}
\R & k=0,n\\
...
\end{equation*}
\begin{itemize}
\item Siccome \(U_{0}\cong U_{1} \cong \R^{n}\) \hyperref[sec:org0bd9510]{diffeomorfi}, \hyperref[sec:orgaa5418d]{allora}\footnote{Vedi anche ``\href{20251115173611-coomologia_di_de_rham_di_r.org}{Coomologia di De Rham di R}''}
\begin{equation*}
  \forall k \in \N: \qquad H^{k}(U_{0}) \cong H^{k}(U_{1}) \cong H^{k}(\R) = \begin{cases}
      ...
      \end{cases}
\end{equation*}
\item \(U_{0}\cap U_{1} = \mathds{S}^{n}\setminus\set{\textbf{N},\textbf{S}} \cong \mathds{S}^{n-1} \times \R\), e \hyperref[sec:org2883e56]{pertanto}
\begin{equation*}
...
\hyperref[sec:org0497431]{Si è già dimostrato} che
\begin{equation*}
H^{k}(\mathds{S}^{1}) = %
\begin{cases}
\R & k = 0,1\\
...
\begin{itemize}
\item \uline{Se \(k=0\)}: Siccome \(\mathds{S}^{n}\) è \href{20250103165325-spazio_topologico_connesso.org}{connesso}, \hyperref[sec:org0c3a6a2]{allora} \(H^{0}(\mathds{S}^{n}) \cong \R\).

\item \uline{Se \(k=1\)}: Si consideri \hyperref[sec:org87251ff]{Mayer-Vietoris}:
\begin{equation*}
...
\item \(H^{0}(U_{0}\cap U_{1}) \cong \R\) \hyperref[sec:org0c3a6a2]{in quanto} connesso
...
Questo \hyperref[sec:orgeb38ceb]{segue} dal fatto che \href{20251223161934-orientabilita_delle_sfere.org}{\(\mathds{S}^{n}\) è orientabile}.

\item \uline{La forma volume non è esatta}

Se per assurdo \(\nu\) fosse esatta, allora \(\nu = \dif \omega\). Poiché \(\mathds{S}^{n}\) è \href{20250103163701-spazio_topologico_compatto.org}{compatta}, allora per il \hyperref[sec:org914a1f5]{Teorema di Stokes}:
\begin{equation*}
...
Siccome la forma volume è una \(n\)-forma, allora è \hyperref[sec:orgc77f16f]{chiusa}, e pertanto \([\nu] \in H^{n}(\mathds{S}^{n})\), e siccome non è esatta allora \([\nu] \neq 0\). Quindi \([\nu]\) genera \(H^{n}(\mathds{S}^{n}) \cong \R\).
\end{proof}

\begin{prop}
Si definisca la mappa data dall'\hyperref[sec:org48ec9eb]{integrale}:
\begin{align*}
...
Infatti \(\int_{\mathds{S}^{n}} \nu > 0\) per \(\nu\) \hyperref[sec:org3cafcbe]{forma volume}. (Questo lo si può fare in quanto \(\mathds{S}^{n}\) è \href{20250103163701-spazio_topologico_compatto.org}{compatta} e \hyperref[sec:orgc238c7b]{orientabile}.)
\end{enumerate}
...
\label{sec:org3cafcbe}
\begin{definizione}
Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile} \(n\)-dimensionale. Una \uline{forma volume} è una \hyperref[sec:org7afd152]{\(n\)-forma} mai nulla \(\nu\).
\end{definizione}
\begin{esempio}
...
\begin{oss}
Se \(\nu\) è una forma volume su \(M\), allora esiste \hyperref[sec:orgc7903cf]{una \(f \in C^{\infty} (U)\)} tale che\footnote{Vedi ``\href{20251201155413-restrizione_di_una_forma_ad_una_sottovarieta.org}{Restrizione di una forma ad una sottovarietà}''}
\begin{equation*}
f \cdot \restriction{\nu}{U} = \dif x^{1}\wedge \dots \wedge \dif x^{n},\qquad f>0.
\end{equation*}
\end{oss}
\begin{prop}
Se \(\nu\) è una forma volume su \(M\), allora l'\hyperref[sec:org48ec9eb]{integrale} è positivo:
\begin{equation*}
\int_{M} \nu > 0.
\end{equation*}
\end{prop}
\begin{proof}
Vedi Osservazione~4.3.6 \autocite{abateGeometriaDifferenziale2011}
\end{proof}
\section{Varietà Differenziabile Orientabile}
\label{sec:orgc238c7b}
\begin{definizione}
...
\label{sec:orgeb38ceb}
\begin{thm}
Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile}. Esiste una \hyperref[sec:org7afd152]{forma} \hyperref[sec:org3cafcbe]{volume} su \(M\) sse \(M\) è \hyperref[sec:orgc238c7b]{orientabile}.
\end{thm}
\section{Supporto di una forma differenziale}
\label{sec:org0a8d5dd}
\begin{definizione}
Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile}, e sia \(\omega\) una \hyperref[sec:org7afd152]{forma differenziale}. Il \uline{supporto di \(\omega\)} è la \href{20250103144944-chiusura_topologica.org}{chiusura topologica}:\footnote{Vedi anche ``\hyperref[sec:org85d06f7]{Forma differenziale in un punto}''}
\begin{equation*}
...
\label{sec:org48ec9eb}
Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile} \hyperref[sec:orgc238c7b]{orientata} \(n\)-dimensionale,
sia \(\omega\) una \hyperref[sec:org7afd152]{forma differenziale}, \(\omega \in\Omega^{n}(M)\) a \hyperref[sec:org0a8d5dd]{supporto} \href{20250103163701-spazio_topologico_compatto.org}{compatto}. Sia \(\set{(U_{\alpha},\varphi_{\alpha})}_{\alpha \in A}\) un \href{20250113103136-atlante_topologico_differenziabile.org}{atlante} \href{20251223153742-atlante_orientato.org}{orientato} di \(M\).
...
\end{equation*}
tramite il \hyperref[sec:orgfd4a0e2]{pullback} \((\varphi^{-1})^{*} : \Omega^{n}(M) \to \Omega^{n}(\R^{n})\).
...
\begin{definizione}
Sia \(\omega \in \Omega^{n}(M)\) una \hyperref[sec:org7afd152]{forma differenziale} a \hyperref[sec:org0a8d5dd]{supporto} \href{20250103163701-spazio_topologico_compatto.org}{compatto}. Se \(\set{(U_{\alpha},\varphi_{\alpha})}_{\alpha \in A}\) e \(\set{\rho_{\alpha}}_{\alpha \in A}\) sono come sopra, allora si pone:
\begin{equation*}
...
\label{sec:org914a1f5}
\begin{thm}
Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differeniabile} \hyperref[sec:orgc238c7b]{orientabile}, e sia \(\omega\) una \(n-1\)-\hyperref[sec:org7afd152]{forma} a \hyperref[sec:org0a8d5dd]{supporto} \href{20250103163701-spazio_topologico_compatto.org}{compatto}. Allora l'\hyperref[sec:org48ec9eb]{integrale}:\footnote{\(\dif\) è il \hyperref[sec:org93adfa7]{differenziale di una forma}.}
\begin{equation*}
\int_{M} \dif \omega = 0.
\end{equation*}
\end{thm}
\section{Teorema di invarianza omotopica per la coomologia di De Rham}
\label{sec:orgcf311c2}
\begin{thm}
Siano \(M,N\) \href{20250113115909-struttura_differenziabile.org}{varietà differenziabili}. Se sono \hyperref[sec:org89ee594]{omotopicamente equivalenti}, allora hanno \hyperref[sec:orga6f5267]{coomologia di De Rham} \hyperref[sec:orgd97769d]{isomorfa} (come \hyperref[sec:org87af76a]{algebra graduata}\footnote{Vedi anche ``\hyperref[sec:orgc9c22e8]{Coomologia di De Rham come algebra graduata}''}):
\begin{equation*}
...
\label{sec:orgf9d56ec}
\begin{thm}
Sia \(\mathds{P}^{n} \R\) lo \href{20241231115051-spazio_proiettivo.org}{spazio proiettivo} reale di dimensione \(n\). La sua \hyperref[sec:orga6f5267]{coomologia di De Rham} è
\begin{equation*}
H^{k}(\mathds{P}^{n}\R) =%
\begin{cases}
\R & k = 0\\
...
\begin{itemize}
\item \hyperref[sec:orgfd4a0e2]{Pullback di una funzione tra varietà differenziabili}
\item \hyperref[sec:orgb35e1d7]{Pullback di una funzione tra varietà differenziabili in coomologia}
\end{itemize}}:
\begin{align*}
...
Inoltre, siccome per definizione di \(\mathds{P}^{n}\R\): \(\pi \circ a = \pi\): \hyperref[sec:org204f778]{allora}
\begin{equation*}
 (\pi \circ a)^{*} = \pi^{*}%
 \IMPLICA %
 a^{*} \circ \pi^{*} = \pi^{*}
...
Dunque, se \(k \neq n\), allora \(H^{k}(\mathds{S}^{n}) = 0\) (per la \hyperref[sec:orge78e440]{coomologia delle sfere}), ma
\begin{equation*}
H^{k}(\mathds{P}^{n}\R) \cong  H^{k}(\mathds{S}^{n})_{+} \subseteq H^{k}(\mathds{S}^{n}) = 0
\end{equation*}
e quindi \(H^{k}(\mathds{P}^{n}\R) = 0\).

Sia ora quindi \(k=n\). La situazione è la seguente\footnote{Vedi ``\hyperref[sec:orge78e440]{Coomologia delle sfere}''}:
\begin{equation*}
H^{n}(\mathds{P}^{n}\R) \cong H^{n}(\mathds{S}^{n})_{+} \subseteq H^{n}(\mathds{S}^{n}) = \langle [\nu] \rangle
\end{equation*}
dove \(\nu\) è la \hyperref[sec:org3cafcbe]{forma volume} su \(\mathds{S}^{n}\). È possibile calcolare che
\begin{equation*}
a^{*}\nu = (-1)^{n+1}\nu
\end{equation*}
e pertanto: \(\bm{a}^{*}[\nu] = (-1)^{n+1}[\nu]\).
...
\end{oss}
\begin{oss}
La \hyperref[sec:org6274754]{coomologia a supporto compatto} è uguale alla \hyperref[sec:org51522b3]{coomologia di complesso di cocatene} di \href{20251229110021-forma_differenziale_a_supporto_compatto.org}{\(A^{\bullet}_{\text{c}}\)}.
\end{oss}
\section{Coomologia a supporto compatto di R}
\label{sec:orga569f87}
\begin{thm}
Se \(H^{k}_{\text{c}}\) indica la \hyperref[sec:org6274754]{coomologia a supporto compatto}, allora
\begin{equation*}
H^{k}_{\text{c}}(\R) = %
\begin{cases}
\R & k = 1\\
...
Per quanto riguarda \(H^{1}_{\text{c}}(\R)\), dimostro che l'\hyperref[sec:org48ec9eb]{integrale}
\begin{align*}
...
Sia \(\omega = g \dif t \in A^{1}_{\text{c}} (\R)\) una \hyperref[sec:org7afd152]{\(1\)-forma} a \hyperref[sec:org0a8d5dd]{supporto} \href{20250103163701-spazio_topologico_compatto.org}{compatto} tale che \(\int_{\R}\omega = 0\). Si definisce
\begin{equation*}
...
\int_{\R} e \dif t = 1.
\end{equation*}
Sia \(\varepsilon \in A^{n}_{\text{c}}(\R^{n})\) una \hyperref[sec:org7afd152]{\(n\)-forma differenziale} a \hyperref[sec:org0a8d5dd]{supporto} \href{20250103163701-spazio_topologico_compatto.org}{compatto}:
\begin{equation*}
\varepsilon \coloneqq %
...
\end{equation*}
Allora l'\hyperref[sec:org48ec9eb]{integrale} \(\int_{\R^{n}} \varepsilon = 1\) e pertanto \([\varepsilon] \in H^{n}_{\text{c}}(\R^{n})\) lo genera.
\end{oss}
\section{Successione di spazi vettoriali esatta induce successione esatta dei duali}
\label{sec:org41bfe8e}
...
\begin{prop}
Se la seguente è una \hyperref[sec:org233b1bc]{successione di spazi vettoriali esatta in \(B\)}:
\begin{equation*}
\begin{tikzcd}
	A & B & C
...
\label{sec:org832d940}
\begin{prop}
Se \(M, N\) sono \href{20250113115909-struttura_differenziabile.org}{varietà differenziabili} \hyperref[sec:org0bd9510]{diffeomorfe}, allora per ogni \(k \in \N\):
\begin{equation*}
H^{k}_{\text{c}}(M) \cong H^{k}_{\text{c}}(M)
\end{equation*}
sono \href{20250113125833-isomorfismo_tra_spazi_vettoriali.org}{isomorfe}, dove \(H^{k}_{\text{c}}\) indica la \hyperref[sec:org6274754]{coomologia a supporto compatto}.
\end{prop}
\section{Estensione a zero di una forma differenziale a supporto compatto}
\label{sec:org3143857}
Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile}, e si considerino le \hyperref[sec:org7afd152]{forme differenziali} \href{20251229110021-forma_differenziale_a_supporto_compatto.org}{a supporto compatto} \(A^{k}_{\text{c}}\).
...
\label{sec:org4548cfe}
Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile}. Si indichi con \(A^{\bullet}_{\text{c}}\) l'insieme delle \href{20251229110021-forma_differenziale_a_supporto_compatto.org}{forme differenziali a supporto compatto} e con \(j_{\star}\) l'\hyperref[sec:org3143857]{estensione a zero} di tali forme.
...
\end{align*}
Queste sono \href{20250114101949-funzione_lineare.org}{funzioni lineari} e \hyperref[sec:org39cbe10]{morfismi tra complessi di cocatene}.

\begin{prop}
(Proposizione~5.5.8 di \autocite{abateGeometriaDifferenziale2011}).
...
\end{tikzcd}
\end{equation*}
è una \hyperref[sec:org233b1bc]{SEC di spazi vettoriali} (e di \hyperref[sec:orgf743f72]{complessi di cocatene}\footnote{Vedi anche ``\hyperref[sec:orgf637da5]{Successione Esatta di Complessi di Cocatene}''}), e pertanto \hyperref[sec:orgdb6d57a]{induce} una \hyperref[sec:org233b1bc]{successione esatta} in \hyperref[sec:org51522b3]{coomologia}:
\begin{equation*}
\begin{tikzcd}
...
\end{tikzcd}
\end{equation*}
Inoltre, anche la \hyperref[sec:org41bfe8e]{successione duale} lo è:
\end{prop}
\begin{oss}
La scrittura di cui sopra si ha in virtù del fatto che la \hyperref[sec:org6274754]{coomologia a supporto compatto} è uguale alla \hyperref[sec:org51522b3]{coomologia di complesso di cocatene} di \href{20251229110021-forma_differenziale_a_supporto_compatto.org}{\(A^{\bullet}_{\text{c}}\)}.
\end{oss}
\begin{proof}
...
\end{equation*}
\end{proof}
\section{Lemma del cinque}
\label{sec:org842279b}
...
\end{lem}
\begin{oss}
Sempre nel caso in cui \(M\) è una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile} \hyperref[sec:orgc238c7b]{orientabile} e \href{20250103163701-spazio_topologico_compatto.org}{compatta}, allora\footnote{Questo si ha perché:
\begin{itemize}
\item \href{20251115191303-varieta_differenziabile_di_tipo_finito.org}{Varietà differenziabile compatta ha tipo finito}
\item \hyperref[sec:org2a2aec9]{Varietà differenziabile di tipo finito ha coomologia di dimensione finita}
...
\label{sec:org3bc06d9}
Si indica con \(H^{k}\) la \hyperref[sec:orga6f5267]{Coomologia di De Rham}, con \(H^{k}_{\text{c}}\) \hyperref[sec:org6274754]{coomologia a supporto compatto}, e con \(\cong\) gli \href{20250113125833-isomorfismo_tra_spazi_vettoriali.org}{isomorfismi}
\begin{thm}
...
\begin{enumerate}
\item Se \(M\) è \uline{\hyperref[sec:orgc238c7b]{orientabile} e \href{20250103163701-spazio_topologico_compatto.org}{compatta}}, allora
\begin{equation*}
 H^{n}(M) \cong \R.
\end{equation*}
\item Se \(M\) è \uline{\hyperref[sec:orgc238c7b]{orientabile} e \href{20250103163701-spazio_topologico_compatto.org}{non compatta}}, allora
\begin{equation*}
 H^{n}(M) = 0.
\end{equation*}
\item Se \(M\) è \uline{\hyperref[sec:orgc238c7b]{non orientabile}}, allora
\begin{equation*}
 H^{n}(M) = 0.
\end{equation*}
\item Se \(M\) è \uline{\hyperref[sec:orgc238c7b]{orientabile}}, allora
\begin{equation*}
...
\begin{equation*}
 H^{n}(M) \cong H^{0}(M)
\end{equation*}
e \hyperref[sec:org0c3a6a2]{siccome} \(M\) connessa
\begin{equation*}
  H^{0}(M) \cong \R.
\end{equation*}
\item Siccome \(M\) è connessa, allora \(H^{0}_{\text{c}}(M) = 0\) (\hyperref[sec:org0c3a6a2]{come per \(H^{0}\)}). Quindi per \href{20251229114152-dualita_di_poincare.org}{DP}
\begin{equation*}
 H^{n}(M) \cong H^{0}_{\text{c}}(M) = 0.
\end{equation*}

\item Come nel calcolo della \hyperref[sec:orgf9d56ec]{coomologia di \(\mathds{P}^{n}\R\)}, per \(M\) non orientabile esiste \(\tilde{M}\) \hyperref[sec:orgc238c7b]{orientabile} \(n\)-dimensionale e
\begin{equation*}
 \pi:\tilde{M} \to M
\end{equation*}
\href{20250103103252-funzione_continua.org}{continua} e \href{20241213105600-funzione_suriettiva.org}{suriettiva} tale che il \hyperref[sec:orgb35e1d7]{pullback}
\begin{equation*}
...
\begin{itemize}
\item il \hyperref[sec:orgfd4a0e2]{pullback}:
\begin{equation*}
...
\end{oss}
\begin{oss}
Se \(M\) è una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile} qualsiasi, allora la \href{20241205142027-spazio_vettoriale.org}{dimensione} di \hyperref[sec:org6274754]{\(H^{n}_{\text{c}}(M)\)} è uguale al numero di \href{20250325160128-componente_connessa_di_uno_spazio_topologico.org}{componenti connesse} \hyperref[sec:orgc238c7b]{orientabili} di \(M\).
\end{oss}
\section{Esistenza forma differenziabile a supporto compatto che integra a uno}
\label{sec:org667d800}

\begin{oss}
Quindi, se \(M\) è una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile} \href{20250103165325-spazio_topologico_connesso.org}{connessa} e \hyperref[sec:orgc238c7b]{orientabile} di dimensione \(n\), esiste \href{20251229110021-forma_differenziale_a_supporto_compatto.org}{\(\omega_{M} \in A^{n}_{\text{c}}(M)\)} tale che
\begin{enumerate}
\item La \hyperref[sec:org6274754]{coomologia a supporto compatto} \(H^{n}_{\text{c}}(M) = \langle [\omega_{M}] \rangle\)\footnote{Vedi ``\href{20250102163502-base_di_uno_spazio_vettoriale.org}{Base di uno spazio vettoriale}''};
\item \(\int_{M} \omega_{M} = 1\).
\end{enumerate}

Infatti, per la \href{20251229114152-dualita_di_poincare.org}{Dualità di Poincaré} si ha il seguente \href{20250113125833-isomorfismo_tra_spazi_vettoriali.org}{isomorfismo} con lo \href{20250105124008-spazio_vettoriale_duale.org}{spazio duale}\footnote{Vedi anche ``\hyperref[sec:orga6f5267]{Gruppo di Coomologia di De Rham}''}:
\begin{equation*}
\begin{tikzcd}
	{H^0(M)} & {H^n_{\text{c}}(M)^*} \\
...
\end{equation*}
\begin{itemize}
\item Siccome \(M\) è connessa, \hyperref[sec:org0c3a6a2]{allora} \(\dim H^{0}(M) = 1 < +\infty\).
...
\begin{itemize}
\item Sicuramente \([\omega_{M}] \in H^{n}_{\text{c}}(M)\), per definizione del \hyperref[sec:orgc9c22e8]{prodotto wedge}.
\item \([\omega_{M}] \neq 0\) (e quindi genera tutto lo spazio): infatti, se per assurdo \([\omega_{M}] = 0\) \hyperref[sec:org45e681c]{allora \(\omega_{M}\) è esatta}, e quindi, per il \hyperref[sec:org914a1f5]{Teorema di Stokes},
\begin{equation*}
  \int_{M} \omega_{M} = 0.
\end{equation*}
Assurdo.
...
\label{sec:orgbca2520}
\begin{prop}
Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile} \hyperref[sec:orgc238c7b]{orientabile}, \href{20250103165325-spazio_topologico_connesso.org}{connessa} e \href{20250103163701-spazio_topologico_compatto.org}{compatta}. Allora una \hyperref[sec:org7afd152]{\(n\)-forma differenziale \(\omega \in A^{n}(M)\)} è \hyperref[sec:org45e681c]{esatta} sse l'\hyperref[sec:org48ec9eb]{integrale}
\begin{equation*}
\int_{M} \omega = 0.
\end{equation*}
\end{prop}
\begin{proof}
(\(\Rightarrow\)): per il \hyperref[sec:org914a1f5]{Teorema di Stokes}.

(\(\Leftarrow\)): Per la \href{20251229114152-dualita_di_poincare.org}{Dualità di Poincaré} (e \hyperref[sec:org3bc06d9]{Coomologia in dimensione massima}), si ha che \(H^{n}(M)\) ha dimensione 1.\footnote{Vedi ``\hyperref[sec:orga6f5267]{Gruppo di Coomologia di De Rham}''} Pertanto:
\begin{align*}
\int_{M}: H^{n}(M) &\longrightarrow \R\\
[\omega] &\longmapsto \int_{M}\omega
\end{align*}
...
\end{equation*}
dove \(i^{*}\) è il \hyperref[sec:orgfd4a0e2]{pullback}.

Se \(\omega\) è esatta, allora \(\omega = \dif \eta\):\footnote{Vedi \hyperref[sec:org204f778]{Proprietà del pullback di una funzione tra varietà differenziabili}}
\begin{equation*}
...
\end{equation*}
per il \hyperref[sec:org914a1f5]{Teorema di Stokes}.
\end{proof}
\section{Coomologia delle superfici topologiche compatte orientabili}
\label{sec:orgd030057}
\begin{thm}
Se \(\Sigma_{g}\) è la \href{20251230172241-superficie_topologica.org}{superficie topologica} \href{20250103163701-spazio_topologico_compatto.org}{compatta} \hyperref[sec:orgc238c7b]{orientabile} di genere \(g\), allora la \hyperref[sec:orga6f5267]{Coomologia di De Rham} è\footnote{Vedi ``\href{20241213095808-somma_diretta.org}{Somma Diretta}''}
\begin{equation*}
...
\end{lem}
\begin{proof}
Dato il Toro \(\Toro\), \hyperref[sec:org5cdc1b0]{di cui si conosce la coomologia}\footnote{Si ha che
\begin{equation*}
\Hk{k}(\Toro) \cong
\begin{cases}
\R & k = 0,2\\
...
O & k\neq 0
\end{cases}
\end{equation*}}. Si noti che \(\ToroBucato\cap D\) è omotopo a \(\mathds{S}^{1}\), \hyperref[sec:org0497431]{di cui si conosce la coomologia}\footnote{Si ha che
\begin{equation*}
...
0 & k \ge 2
\end{cases}
\end{equation*}}. Si scrive quindi \hyperref[sec:org87251ff]{Mayer-Vietoris}:
\begin{equation*}
...
\begin{itemize}
\item siccome tutti gli spazi sono connessi, \hyperref[sec:org0c3a6a2]{ogni \(\Hk{0}\) è isomorfo a \(\R\)} (in particolare, \(\boxed{\Hk{0}(\ToroBucato) \cong \R}\));
\item siccome \(\ToroBucato\) è \hyperref[sec:orgc238c7b]{orientabile} ma \href{20250103163701-spazio_topologico_compatto.org}{non compatta}, \hyperref[sec:org3bc06d9]{allora} \(\boxed{\Hk{2}(\ToroBucato) = 0}\);
...
\uline{Caso \(k=0\)}: siccome \(\Sigma_{g}\) è \href{20250103165325-spazio_topologico_connesso.org}{connessa}, \hyperref[sec:org0c3a6a2]{allora} \(\boxed{\Hk{0}(\Sigma_{g}) = \R}\).
...
\item per \(k=2\): siccome \(\Sigma_{g}\) è compatta e orientabile, allora (\hyperref[sec:org3bc06d9]{Coomologia in dimensione massima})
\begin{equation*}
...
\begin{itemize}
\item \(U\) è \hyperref[sec:org89ee594]{omotopo} alla superficie \(\tilde{\Sigma}_{g-1} \coloneqq \Sigma_{g-1} \setminus \set{\text{punto}}\);
...
Siccome si vuole utilizzare la \hyperref[sec:org87251ff]{sequenza di Mayer-Vietoris}, (sfruttando l'\hyperref[sec:orgcf311c2]{invarianza della coomologia per omotopia}) è necessario calcolare la coomologia di \(\tilde{\Sigma}_{g-1}\). Si procede per induzione
\begin{itemize}
...
\item Si conoscono la coomologia \hyperref[sec:org0497431]{di \(\mathds{S}^{1}\)} e \(\ToroBucato\)\footnote{Vedi il Lemma~\ref{lem:coomtorobuc}}, nonché le coomologie di dimensione \(0\) e \(2\) si \(\Sigma_{g}\);
\item \(\tilde{\Sigma}_{g-1}\) è una superficie orientabile non compatta, e \hyperref[sec:org3bc06d9]{quindi} \(\Hk{2}(\tilde{\Sigma}_{g-1}) \cong 0\).
\end{itemize}
Resta quindi:
\begin{equation*}
\begin{tikzcd}[column sep=small]
...
\item \(\Hk{k}(A \cap B) \cong \Hk{k}(\mathds{S}^{1})\), \hyperref[sec:org0497431]{coomologia nota}.
\end{itemize}
...
\label{sec:org3799df8}
\begin{prop}
Sia \(M\) una \href{20250113115909-struttura_differenziabile.org}{varietà differenziabile} \hyperref[sec:orgc238c7b]{orientabile}, \href{20250103165325-spazio_topologico_connesso.org}{connessa}. Allora una \hyperref[sec:org7afd152]{\(n\)-forma differenziale \(\omega \in A^{n}_{\text{c}}(M)\)} a \href{20251229110021-forma_differenziale_a_supporto_compatto.org}{supporto compatto} è \hyperref[sec:org45e681c]{esatta} sse l'\hyperref[sec:org48ec9eb]{integrale}
\begin{equation*}
\int_{M} \omega = 0.
\end{equation*}
\end{prop}
...
(\(\Leftarrow\)): Per la \href{20251229114152-dualita_di_poincare.org}{Dualità di Poincaré} (e \hyperref[sec:org3bc06d9]{Coomologia in dimensione massima}), si ha che \hyperref[sec:org6274754]{\(H^{n}_{\text{c}}(M)\)} ha dimensione 1. Pertanto:
\begin{align*}
...
\label{sec:org410173b}
\section{Teorema di Sard}
\label{sec:org46dffab}
\section{Teorema del Grado}
\label{sec:org7716712}
\end{document}
