...
\date{\today}
\title{Funzione discriminatoria per una famiglia di misure}
\begin{document}

\section{Funzione discriminatoria (Machine Learning)}
\label{sec:org720705b}
\begin{definizione}
...
\end{equation*}
Si vedano ``\href{20250624162220-spazi_lp.org}{Spazi Lp}''\label{org093cc71}} e
\begin{equation*}
...
\item se \(g \in L^{\infty}(I_{n})\)\textsuperscript{\ref{org093cc71}} e
\begin{equation*}
 \forall \bm{w} \in \R^{n}\ \forall \theta \in \R:\qquad\int_{I_{n}} \sigma(\bm{w}\cdot\bm{x} + \theta)g(\bm{x})\dif\bm{x} = 0
\end{equation*}
allora \(g=0\)
