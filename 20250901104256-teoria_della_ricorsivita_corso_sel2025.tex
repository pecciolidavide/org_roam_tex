...
\title{Teoria della ricorsività [CORSO SEL2025]}
\begin{document}

\begin{latex}
\title{Teoria della ricorsività}
\maketitle
\end{latex}
\section{Lezione 1}
\label{sec:org4f297a4}
...
\end{quote}
\section{Lezione 2}
\label{sec:orgf6e3843}

\subsection{Turing's computability}
\label{sec:org8d0f323}
...
\section{Lezione 3}
\label{sec:org19d4069}

\subsection{Conseguenze di CTT}
\label{sec:org455181f}
...
\subsection{Teorema di Punto fisso di Kleene}
\label{sec:orgbf145f0}
...
per ogni \(y\).
\end{proof}
\subsubsection{Dimostrazione del Teorema di Punto Fisso}
\label{sec:org7991364}
...
\subsubsection{Busy Beaver}
\label{sec:orgdd0ef34}
...
Questo implica che:
\begin{itemize}
\item se GC è falsa, allora \(\mathr{BB}(27)\) è estremo superiore del più grande controesempio.
\end{itemize}
...
\end{esempio}
\subsection{Computable enumeration}
\label{sec:orgdecbd0a}
...
Assurdo.
\end{proof}
\subsection{Riducibilità \emph{many-one}.}
\label{sec:orgc63a3ee}
...
\end{cases}
\end{equation*}
Se \(x \in A\), allora \(\varphi_{g(x)}\big(g(x)\big)\downward\), e pertanto \(g(x) \in K\). Se \(x\notin A\), allora \(\varphi_{g(x)}\big(g(x)\big)\uparrow\), e dunque \(g(x)\notin K\).
...
\end{itemize}
\end{proof}
\subsection{Teorema di Godel}
\label{sec:org92d68c2}
